...
- **マルチプロバイダー** — DeepSeek / OpenAI / Gemini を環境変数またはDiscordコマンドで切り替え
- **ルーター** — Gemini で質問を `simple` / `complex` に分類（DeepSeek / OpenAIにも変更可能）
...
[History] 直近50件の会話履歴取得
    ...
    └─ complex ─▶ [Planner] 調査計画立案
                      │
                      ▼
                  [Tool Loop] Main AI + ツール呼び出し（最大15回）
                      │
                      ▼
...
デフォルトではメインAIは `AI_PROVIDER` / `AI_MODEL`、ルーターは `ROUTER_PROVIDER` / `ROUTER_MODEL` で決まります。起動後はDiscordの管理者向けスラッシュコマンドでも、サーバーごとに切り替えられます。
...
/ai router gemini
/ai router-model gemini-2.5-flash-lite
/ai reset
```
...
```bash
ALLOWED_GUILDS=1507221943248490566
# または
IGNORED_GUILDS=1504090376162770986
```
...
| `AI_PROVIDER` | | メインAIプロバイダー: `deepseek` / `openai` / `gemini`（デフォルト: `deepseek`） |
| `AI_MODEL` | | メインAIモデルの上書き |
| `ROUTER_PROVIDER` | | ルーター用プロバイダー（デフォルト: `gemini`） |
| `ROUTER_MODEL` | | ルーターモデルの上書き |
| `DEEPSEEK_API_KEY` | | DeepSeek APIキー |
| `OPENAI_API_KEY` | | OpenAI APIキー |
| `GEMINI_API_KEY` | ✓ | Gemini APIキー（デフォルトルーターで使用） |
| `ALLOWED_GUILDS` | | 指定したDiscordサーバーだけで応答 |
| `IGNORED_GUILDS` | | 指定したDiscordサーバーでは応答しない |
...
- **Multi-provider AI** — Switch DeepSeek / OpenAI / Gemini via environment variables or Discord commands
- **Router** — Classifies each message as `simple` or `complex` using Gemini by default, with DeepSeek / OpenAI available
...
[History] Fetch last 50 messages
    │
    ├─ simple ──▶ [Main AI] Direct answer
    │
    └─ complex ─▶ [Planner] Build investigation plan
                      │
                      ▼
                  [Tool Loop] Main AI + tool calls (up to 15 iterations)
                      │
                      ▼
...
By default, the main AI is controlled by `AI_PROVIDER` / `AI_MODEL`, and the router is controlled by `ROUTER_PROVIDER` / `ROUTER_MODEL`. After startup, Discord administrators can change these settings per guild with slash commands.
...
If the same Discord application is installed in multiple servers, restrict where this bot responds with `.env`.
...
| `AI_PROVIDER` | | Main AI provider: `deepseek` / `openai` / `gemini` (default: `deepseek`) |
| `AI_MODEL` | | Main AI model override |
| `ROUTER_PROVIDER` | | Router provider (default: `gemini`) |
| `ROUTER_MODEL` | | Router model override |
| `DEEPSEEK_API_KEY` | | DeepSeek API key |
| `OPENAI_API_KEY` | | OpenAI API key |
| `GEMINI_API_KEY` | ✓ | Gemini API key, used by the default router |
| `ALLOWED_GUILDS` | | Only respond in these Discord guild IDs |
| `IGNORED_GUILDS` | | Do not respond in these Discord guild IDs |
