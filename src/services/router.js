...
async function classifyOnce(userMessage, serverInfo) {
...
export async function classify(userMessage, serverInfo = '') {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const result = await classifyOnce(userMessage, serverInfo);
      console.log(`[router] ${result.type} (attempt ${attempt})`);
      return result;
    } catch (e) {
      lastError = e;
