...
与えられたリサーチクエリと、Discordから収集したユーザー情報をもとに、ウェブ検索の調査計画を立ててください。

【最重要: ユーザー情報の活用】
Discord情報にはユーザーのスキル・進行中プロジェクト・興味関心が含まれています。
検索クエリを作る際は、必ずそのユーザーの具体的なスキルや状況と、リサーチクエリを組み合わせてください。
例:
  - ユーザーがWeb開発者で「収入を得たい」→ "web developer passive income SaaS 2026" のように具体化
  - ユーザーがAI実装スキルを持っている → "AI chatbot monetization Gumroad" のようにスキルを活かす検索に
  - ユーザーが特定のプラットフォーム運営中 → そのプラットフォーム名を含めた検索
汎用的な「～したい 方法」だけでなく、ユーザーの強みを活かした具体的な検索キーワードを作ること。
...
- subQuestions: リサーチクエリを、ユーザーの状況に即して具体化したサブ質問（3〜7個）
- searchQueries: ユーザーのスキル・資産を織り込んだ具体的な検索キーワード（5〜10個、多角的に）
- angles: 調査すべき観点や切り口（例: 技術スタックを活かす方法、匿名販売の仕組み、成功事例など）`;
